...
def drawTree2(blnCompact):
    ...
    def go(blnPruned, tree):
        ...
        levelWidths = reduce(
            lambda a, xs: a + [max(x[0] for x in xs)], levels(measuredTree), []
        )
        treeLines = stringsFromLMR(foldr(lmrBuild)(None)(levelWidths)(measuredTree))
        return (
            [s for s in treeLines if any(c not in "│ " for c in s)]
            if (not blnCompact and blnPruned)
            else treeLines
        )
...
def intercalate(x):
    ...
    return (
        lambda xs: x.join(xs)
        if isinstance(x, str)
        else list(chain.from_iterable(reduce(lambda a, v: a + [x, v], xs[1:], [xs[0]])))
        if xs
        else []
    )
...
def levels(tree):
    ...
    return list(map_(map_(root))(takewhile(bool, iterate(concatMap(nest))([tree]))))
